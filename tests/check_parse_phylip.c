...
START_TEST (phylip_read_in_small_file)
{
  ...
  ck_assert_msg( number_of_samples_from_parse_phylip() == 3);
  ck_assert_msg( find_sequence_index_from_sample_name("2956_6_1") == 0);
  ck_assert_msg( find_sequence_index_from_sample_name("2956_6_2") == 1);
  ck_assert_msg( find_sequence_index_from_sample_name("2956_6_3") == 2);
  
  char *sample_names[3];
  get_sample_names_from_parse_phylip(sample_names);
  ck_assert_msg( strcmp(sample_names[0],"2956_6_1") == 0 );
  ck_assert_msg( strcmp(sample_names[1],"2956_6_2") == 0 );
  ck_assert_msg( strcmp(sample_names[2],"2956_6_3") == 0 );
  ...
  ck_assert_msg( strcmp(filtered_bases_for_snps[0], "AAT") == 0 );
  ck_assert_msg( strcmp(filtered_bases_for_snps[1], "CGT") == 0 );
  ck_assert_msg( strcmp(filtered_bases_for_snps[2], "GGT") == 0 );
  

  
  ck_assert_msg( does_column_contain_snps(0, 'A') == 0);
  ck_assert_msg( does_column_contain_snps(1, 'A') == 1);
  ck_assert_msg( does_column_contain_snps(2, 'A') == 1);
  // bad reference base
  ck_assert_msg( does_column_contain_snps(0, 'X') == 1);
  
  char sequence_bases[10];
  get_sequence_for_sample_name(sequence_bases, "2956_6_2");
  ck_assert_msg( strcmp(sequence_bases, "AAGGC") == 0);
  
  update_sequence_base('X', 1, 4);
  get_sequence_for_sample_name(sequence_bases, "2956_6_2");
  ck_assert_msg( strcmp(sequence_bases, "AAGGX") == 0);
...
START_TEST (phylip_read_in_file_with_gaps)
{
	...
	ck_assert_msg( does_column_contain_snps(0, 'A') == 0);
  ck_assert_msg( does_column_contain_snps(1, '-') == 0);
  ck_assert_msg( does_column_contain_snps(2, '-') == 0);
  ck_assert_msg( does_column_contain_snps(3, 'T') == 0);
  ck_assert_msg( does_column_contain_snps(4, 'G') == 1);
  ck_assert_msg( does_column_contain_snps(4, '-') == 1);
  ck_assert_msg( does_column_contain_snps(5, 'N') == 0);
  ...
  ck_assert_msg( strcmp(sequence_bases, "A-N-CT") == 0);
}
END_TEST
...
START_TEST (phylip_fill_in_unambiguous_bases_in_parent_from_children_where_parent_has_a_gap)
{
  ...
  ck_assert_msg( strcmp(sequence_bases, "AC--") == 0);
}
END_TEST
