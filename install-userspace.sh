...
#
# Blame Aidan Delaney <aidan@ontologyengineering.org> for issues with this
# script.
#
...
function check_platform {
    ...
    case $DISTRIB_DESCRIPTION in
        "Ubuntu 13.10"(
            ...
            ;;
        *)
            echo "This setup script only works on Ubuntu 13.10"
            exit
            ;;
    esac
    ...
    case "$(uname -m)" in
        "x86_64")
            ...
            ;;
        *)
            echo "This setup script only works on Ubuntu 13.10 x86_64"
            exit
            ;;
    esac
}
...
function check_dependencies {
    ...
    *)
    ...
    if [ $? -eq 0 ] ; then
        ...
        echo "No Python.h on this system"
        exit 1
    fi
