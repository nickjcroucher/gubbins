    ...
    strcpy(root->taxon_names, "");
      ...
      strcat(root->taxon_names, child_nodes[child_counter]->taxon_names);
		...
		for(current_branch = 0 ; current_branch< (root->childNum); current_branch++)
		{
			...
			print_branch_snp_details(branch_snps_file_pointer, root->taxon, child_nodes[current_branch]->taxon, branches_snp_sites[current_branch], number_of_branch_snps, branch_snp_sequence, branch_snp_ancestor_sequence,root->taxon_names);
